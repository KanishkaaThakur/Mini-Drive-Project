  ...
  try {
    ...
    if (!file.user) {
      if (requestor.role === "admin") {
        ...
        return res
          .status(401)
          .json({ message: "Only Admins can delete ghost files" });
      }
    }
    // -------------------------------------
...
router.get("/shared/:id", async (req, res) => {
  try {
    ...
    if (!file || !file.isPublic) {
      return res
        .status(403)
        .json({ message: "This file is private or does not exist." });
    }
    res.json(file);
  } catch (err) {
    res.status(500).json({ message: "Server error" });
...
router.post("/share-email/:id", auth, async (req, res) => {
  try {
    ...
    if (file.user.toString() !== req.user.id) {
      return res
        .status(401)
        .json({ message: "Not authorized to share this file" });
    }
    ...
    if (!targetUser) {
      ...
      return res
        .status(404)
        .json({ message: `User ${email} not found. Have they registered?` });
    }
